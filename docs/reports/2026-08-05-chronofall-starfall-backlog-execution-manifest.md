...
| `SF-M-LIFE` | Player Defeat and Town Respawn | A monster defeats the admitted player; after authoritative respawn resolution that player returns to the town anchor with restored resources. Entity identity reuse or replacement remains an explicit Player Life Plan-mode decision. |
...
| `CLIENT-0011` | Retitle/refocus to one rendered socketed Basic bow. Remove equipment, `CONTENT-0009`, `GAME-0005`, grip/IK/aim ownership, and starter-loadout wording. Consume exact selected/staged bow input and canonical `SHARED-0020`. |
...
| `CONTENT-0012`, `EDITOR-0007`–`EDITOR-0010`, `SERVER-0012`, `CLIENT-0016` | Remove from the active M2 path and set `priority: none`. Re-groom into a proper-scene deliverable only after gameplay evidence or authoring friction justifies it. `EDITOR-0009` loses its misleading `high` priority. |
...
| `SF-LIFE-MANA-INTEGRATION` | SIM / `SF-M-LIFE` | completed `SIM-0011` behavior plus completed Mana life-cycle seam | Inspect and reuse the existing player damage/defeat/delay/town-respawn/full-health behavior. Add only the missing Mana lifecycle integration and freeze any unresolved cross-resource policy. Do not reimplement completed player-life behavior. Entity reuse versus replacement remains undecided. |
| `SF-LIFE-PROTOCOL` | PROTOCOL / `SF-M-LIFE` | `SF-LIFE-MANA-INTEGRATION` | Encode only the player-life facts still required for connected presentation: admitted-player defeat, respawn timing/anchor, resource restoration, authoritative identity outcome, and corrections. The Client never selects the respawn entity identity. |
...
| --- | --- | --- | --- |
| `CF-DEBUG-CLIENT-ADOPTION` | SHARED / Starfall development debug UI enablement | `SHARED-0016`, `SHARED-0024` | Extend the approved consumer boundary from native Editor-only to Starfall.Client development instrumentation; document/validate exact family-source allowlist, caller-owned SDL/GPU lifecycle, hidden-debug behavior, macOS ARM64 native use, and headless exclusion. Do not build Starfall windows or permanent UI. |
...
- keep `ASSET-0004`, `ASSET-0006`, and `SHARED-0020` on the Basic Arrow enabler path through refocused `CONTENT-0011`;
...
Cycle 2 creates one focused Basic Arrow shared-enabler milestone for `ASSET-0004`, `ASSET-0006`, and `SHARED-0020`. Its observable result is the exact selected archer/bow cook and one rendered socketed static bow proof. It must not absorb Starfall integration or gameplay. Existing task priorities are changed only when the grooming plan makes a deliberate scheduling decision; milestone assignment is not a reason to normalize them cosmetically.
...
- whether Player Life respawn preserves the current authoritative entity identity or allocates a replacement while retaining the admitted player/session relationship;
