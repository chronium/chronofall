...
Approved child source consumption remains narrow and audience-specific. Client presentation may consume only the approved character-presentation projects, while headless simulation may consume only `ChronoFall.Box3D`; its bindings remain transitive. Use `ChronoFallFamilyRoot`; never scatter literal parent traversal, absolute checkout paths, arbitrary external roots, or child-to-child references. SDL3-CS remains coordinator-pinned source compiled transitively through the shared SDL GPU project. Box3D remains coordinator-pinned source compiled into explicit macOS ARM64 and Linux x64 native layouts. Do not replace either boundary with packages or feeds without later task-owned evidence.
